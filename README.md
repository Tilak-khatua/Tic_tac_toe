...
Game Flow
Start: The game initializes with currentPlayer = 'X' and an empty board. The turn indicator shows “Player X’s turn.”
Player Move: When a player clicks an empty cell, placeMark(index) runs. It updates the board state and the clicked cell’s content to “X” or “O.”
Check Win/Draw: After each move, the code checks for a winning combination. If found, it displays “Player X wins!” or “Player O wins!” and stops further moves. If all cells are filled with no winner, it displays “It’s a draw!”.
Switch Turns: If the game is still active (no win or draw), the code switches currentPlayer to the other player and updates the turn text (e.g., “Player O’s turn”).
Reset: Clicking the Reset button clears the board, resets the turn to “Player X,” and allows a new game to start.
